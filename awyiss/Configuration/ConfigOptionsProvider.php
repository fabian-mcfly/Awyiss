...
class ConfigOptionsProvider {
	 ...
	 * @noinspection PhpUnused
	 */
	public static function validateConfigValue (string $as_scopeName, string $as_configOptionName, mixed $ax_value, ?string $as_languagesShortcode = NULL): bool|string {
		$lo_configuration = static::loadConfiguration($as_scopeName);
		if (!$lo_configuration) return FALSE;

		return $lo_configuration->validateConfigValue($as_configOptionName, $ax_value, $as_languagesShortcode);
	}
