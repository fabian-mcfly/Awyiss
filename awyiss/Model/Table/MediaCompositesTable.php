...
/**
 * MediaComposites Model
 *
 * @method \Awyiss\Model\Entity\MediaComposite newDefaultEntity(array $additionalData = [], array $options = [])
 * @property \Awyiss\Model\Table\MediaAssignmentsTable&\Awyiss\ORM\Association\HasMany $MediaAssignments
 * @property \Awyiss\Model\Table\MediaCompositeSelectorsTable&\Awyiss\ORM\Association\HasMany $MediaCompositeAssignments
 ...
 */
class MediaCompositesTable extends Table {
	/**
	 * @inheritDoc
	 */
	 ...
	 * @inheritDoc
	 */
	public const TABLE = 'media_composites';


	/**
	 * @var array The available models for composite assignment
	 */
	...
	public function initializeAssociations(): void {
		...
		$this->hasMany('MediaCompositeAssignments', [
			'cascadeCallbacks' => true,
			'dependent' => true,
			'saveStrategy' => 'replace',
		]);
	...
	/**
	 * Get the available models for composite assignment
	 * Model level means that the model is assignable to a composite
	 * Entity level means that the entity of the model is assignable to a composite
	 *
	...
	public function getAssignableModels(bool $includeEntities = false, bool $allowGrouping = true): array {
		...
		foreach ($la_paths as $ls_namespace => $ls_path) {
			...
			foreach (glob($ls_path) as $ls_filePath) {
				...
				$lb_entityLevel = (bool)($lo_attributeInstance->level & MediaCompositeAssignable::ENTITY_LEVEL);
