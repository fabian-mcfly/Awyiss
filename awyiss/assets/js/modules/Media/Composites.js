...
export default class Composites {
	constructor() {
		new this.sortableMediaCompositeAssignments();
	}

	sortableMediaCompositeAssignments = class {
		/**
		 ...
		 * @type {string}
		 */
		sortableListSelector = '.MediaComposites-List';

		constructor() {
			if (document.documentElement.classList.contains('MediaCompositesController')) {
				return;
			}
		...
		onEnd(event) {
			...
			setTimeout(function () {
				checkbox.checked = !targetFieldset.classList.contains('Fieldset-MediaComposites-Available');
			}, 50);
