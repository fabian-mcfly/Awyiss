...
(function () {
	...
	const wrapCharWithSpan = function (value) {
		const classFragment = value === '\u202f' ? (charMap[value] + ' mce-nbsp') : charMap[value];
		return '<span data-mce-bogus="1" class="mce-' + classFragment + '">' + value + '</span>';
	};
	...
	const register$2 = (editor, toggleState) => {
		editor.addCommand('mceVisualCharsPlus', () => {
	...
	const setup = (editor, toggleState) => {
		...
		editor.on('keydown', (e) => {
			...
			debouncedToggle.throttle();
		});

		// Handle input to normalize nbsp spans when user continues typing
		editor.on('input', () => {
			if (toggleState.get() !== true) {
				return;
			}

			// Check if there's an nbsp span before cursor and normalize it
			...
			// Look for nbsp span before the current cursor position
			let checkNode = container;
			if (container.nodeType === 3 && container.previousSibling) {
				checkNode = container.previousSibling;
			} else if (container.nodeType === 1 && selection.startOffset > 0 && container.childNodes[selection.startOffset - 1]) {
				checkNode = container.childNodes[selection.startOffset - 1];
			} else {
				return;
			}

			// If the node before is an nbsp span, normalize it
			if (checkNode && checkNode.nodeType === 1 && checkNode.classList && checkNode.classList.contains('mce-nbsp')) {
				const textContent = checkNode.textContent;
				if (textContent === '\u00a0') {
					// Replace nbsp with normal space
					const textNode = editor.getDoc().createTextNode(' ');
					const parent = checkNode.parentNode;
					parent.replaceChild(textNode, checkNode);

					// Merge with adjacent text nodes if possible
					...
					} else if (textNode.nextSibling && textNode.nextSibling.nodeType === 3) {
	...
	const register = (editor, toggleState) => {
		const onAction = () => editor.execCommand('mceVisualCharsPlus');
		editor.ui.registry.addToggleButton('visualCharsPlus', {
		...
		editor.ui.registry.addToggleMenuItem('visualCharsPlus', {
	...
	var Plugin = () => {
		global.add('visualCharsPlus', (editor) => {
			register$1(editor);
