...
class ConfigOptionsProvider {
	 ...
	 * @noinspection PhpUnused
	 */
	public static function loadConfiguration (string $as_name): ?AbstractConfigOptions {
		$ls_name = Inflector::underscore($as_name);
		...
		if (!$ls_configurationClass) {
			static::$loadedConfigurations[ $ls_name ] = FALSE;
			return NULL;
		}

		static::$loadedConfigurations[ $ls_name ] = TRUE;

		return new $ls_configurationClass();
	}
	...
	protected static function findConfiguration (string $as_name): array {
		...
		$la_paths = [
			'\\' . CUSTOM_NAMESPACE . '\ConfigOptions\\' => implode(DS, [ROOT, CUSTOM_DIR, 'ConfigOptions', $ls_name . 'ConfigOptions.php',]),
			'\Awyiss\ConfigOptions\\' => implode(DS, [ROOT, APP_DIR, 'ConfigOptions', $ls_name . 'ConfigOptions.php']),
		];

		foreach ($la_paths as $ls_namespace => $ls_path) {
			foreach (glob($ls_path) as $ls_filePath) {
				$ls_configurationName = substr($ls_filePath, strrpos($ls_filePath, DS) + 1, -4);
				if ($ls_configurationName === 'AbstractConfigOptions') continue;
				$ls_configurationClass = $ls_namespace . $ls_configurationName;

				$lo_reflection = new ReflectionClass($ls_configurationClass);

				if ( ! $lo_reflection->isSubclassOf(AbstractConfigOptions::class)) {
					throw new RuntimeException(sprintf('The provided Configuration class `%s` does not extend the `%s` class.', $ls_configurationClass, AbstractConfigOptions::class));
				}

				/**
				 * @var AbstractConfigOptions $ls_configurationClass
				 */
