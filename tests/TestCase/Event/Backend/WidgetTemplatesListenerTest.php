...
use ArrayObject;
use Awyiss\Event\Backend\WidgetTemplatesListener;
...
/**
 * WidgetTemplatesListener Test Case
 *
 * @see \Awyiss\Event\Backend\WidgetTemplatesListener
 */
class WidgetTemplatesListenerTest extends TestCase {
	/**
	 * @var \Awyiss\Event\Backend\WidgetTemplatesListener
	 */
	protected WidgetTemplatesListener $listener;
	...
	protected function setUp(): void {
		...
		$this->listener = new WidgetTemplatesListener();
	}
	...
	public function testBeforeMarshalWithEmptyElements(): void {
		...
		$event = new Event('Model.WidgetTemplates.beforeMarshal');

		$this->listener->beforeMarshal($event, $data, $options);

		$this->assertArrayNotHasKey('widget_template_elements', $data);

		$data = new ArrayObject(['widget_template_elements' => null]);
		$options = new ArrayObject();
		$event = new Event('Model.WidgetTemplates.beforeMarshal');

		$this->listener->beforeMarshal($event, $data, $options);

		$this->assertNull($data['widget_template_elements']);
	}
	...
	public function testBeforeMarshalWithTitleAndSubtitlePresent(): void {
		...
		$data = new ArrayObject(['widget_template_elements' => $elements]);
		$options = new ArrayObject();
		$event = new Event('Model.WidgetTemplates.beforeMarshal');
		...
		$this->assertCount(5, $data['widget_template_elements']);
		$identifiers = array_column($data['widget_template_elements'], 'identifier');
	...
	public function testBeforeMarshalWithTitleButNoSubtitle(): void {
		...
		$this->assertCount(3, $data['widget_template_elements']);
		$identifiers = array_column($data['widget_template_elements'], 'identifier');
		$this->assertContains('title', $identifiers);
		$this->assertContains('title_tag', $identifiers);
	...
	public function testBeforeMarshalWithNeitherTitleNorSubtitle(): void {
		...
		$this->assertCount(1, $data['widget_template_elements']);
		$identifiers = array_column($data['widget_template_elements'], 'identifier');
		$this->assertNotContains('title_tag', $identifiers);
		$this->assertNotContains('subtitle_tag', $identifiers);
		$this->assertContains('other_element', $identifiers);
	...
	public function testBeforeSaveWithNoFileNameChange(): void {
		...
		$event = new Event('Model.WidgetTemplates.beforeSave');
	...
	public function testBeforeSaveWithFileNameChangeAndNoQueuedJob(): void {
		...
		$queueTable->expects($this->once())->method('isQueued')->with('widget_templates::file_changes')->willReturn(false);
	...
	public function testBeforeSaveWithFileNameChangeAndQueuedJob(): void {
		...
		$queueTable->expects($this->once())->method('isQueued')->with('widget_templates::file_changes')->willReturn(true);
		...
		$this->assertSame(['widget_templates::file_changes_in_progress'], $errors['_general']);
	}
	...
	public function testAfterSaveCommitWithNewEntity(): void {
		...
		$event = new Event('Model.WidgetTemplates.afterSaveCommit');
		...
		$queueTable->expects($this->once())->method('createJob')->with(
			...
			$this->callback(function ($data) {
				return isset($data['command']) && $data['command'] === implode(' && ', [
						'mkdir -m 0755 -p ' . TMP . 'test_templates/Frontend/widget/',
						'bin/cake bake template widget_templates widget_template test_template --prefix Frontend --controller widget',
						'chmod 0755 ' . TMP . 'test_templates/Frontend/widget/test_template.twig',
					]);
			}),
			[
				'group' => 'general',
	...
	public function testAfterSaveCommitWithNewEntityAndExistingFile(): void {
		...
		$existingFilePath = TMP . 'test_templates/Frontend/widget/test_template.twig';
	...
	public function testAfterSaveCommitWithExistingEntityAndFileNameChange(): void {
		...
		$queueTable->expects($this->once())->method('createJob')->with(
			...
			$this->callback(function ($data) {
				return isset($data['command']) && $data['command'] === implode(' && ', [
						'mkdir -m 0755 -p ' . TMP . 'test_templates/Frontend/widget/',
						'bin/cake bake template widget_templates widget_template new_template --prefix Frontend --controller widget',
						'chmod 0755 ' . TMP . 'test_templates/Frontend/widget/new_template.twig',
					]);
			}),
			[
				'group' => 'general',
	...
	public function testAfterSaveCommitWithExistingEntityAndFileNameChangeAndExistingOldFile(): void {
		...
		$queueTable->expects($this->once())->method('createJob')->with(
			'Queue.Execute',
			$this->callback(function ($data) {
				return isset($data['command']) && $data['command'] === 'mv -f ' . TMP . 'test_templates/Frontend/widget/test_template.twig ' . TMP . 'test_templates/Frontend/widget/new_template.twig';
			}),
			[
				'group' => 'general',
				'priority' => 1,
	...
	public function testAfterSaveCommitWithExistingEntityAndFileNameChangeAndExistingNewFile(): void {
		...
		// Create the file to simulate existing file
		$existingFilePath = TMP . 'test_templates/Frontend/widget/new_template.twig';
		if (!is_dir(dirname($existingFilePath))) {
			mkdir(dirname($existingFilePath), 0755, true);
		}
		file_put_contents($existingFilePath, 'Existing content');
	...
	public function testAfterSaveCommitWithExistingEntityAndFileNameChangeAndExistingOldFileAndExistingNewFile(): void {
		...
		$existingOldFilePath = TMP . 'test_templates/Frontend/widget/test_template.twig';
		...
		$existingNewFilePath = TMP . 'test_templates/Frontend/widget/new_template.twig';
	...
	public function testAfterSoftDeleteWithExistingFile(): void {
		...
		$event = new Event('Model.WidgetTemplates.afterSoftDelete');
		...
		$queueTable->expects($this->once())->method('createJob')->with(
			...
			$this->callback(function ($data) {
				return isset($data['command']) &&
					str_starts_with($data['command'], 'mv -f ' . TMP . 'test_templates/Frontend/widget/test_template.twig ' . TMP . 'test_templates/Frontend/widget/_deleted-test_template-') &&
